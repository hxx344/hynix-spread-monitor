...
assert.equal(existsSync("/etc/hynix-spread.env"), false, "Do not overwrite an existing installation");
assert.equal(existsSync("/opt/hynix-spread"), false, "Do not overwrite an existing installation");
const exec = promisify(execFile);
const root = process.cwd();
const installer = join(root, "deploy/install.sh");
const scratch = await mkdtemp(join(tmpdir(), "hynix-installer-test-"));
...
async function current() { return (await run("readlink", ["-f", "/opt/hynix-spread/current"])).stdout.trim(); }
async function active() { await run("systemctl", ["is-active", "--quiet", "hynix-spread.service"]); }
async function config() { return (await run("sudo", ["cat", "/etc/hynix-spread.env"])).stdout; }
...
try {
  ...
  const pidBefore = (await run("systemctl", ["show", "--property=MainPID", "--value", "hynix-spread.service"])).stdout;
  await install(failedBuild, false);
  assert.equal(await current(), secondRelease);
  assert.equal((await run("systemctl", ["show", "--property=MainPID", "--value", "hynix-spread.service"])).stdout, pidBefore);
  await active();
  assert.equal(await config(), originalConfig);
  ...
  startupManifest.scripts["build:linux"] = "cp -a /opt/hynix-spread/current/.next .next";
  ...
  const unitBefore = await readFile("/etc/systemd/system/hynix-spread.service", "utf8");
  await install(failedStart, false);
  assert.equal(await current(), secondRelease);
  assert.equal(await readFile("/etc/systemd/system/hynix-spread.service", "utf8"), unitBefore);
  await ready(); await active();
  assert.equal(await config(), originalConfig);
  assert.deepEqual((await state()).config.rules, rules);
  assert.equal((await state()).revision, savedState.revision);
  const directories = (await run("find", ["/opt/hynix-spread/releases", "-mindepth", "1", "-maxdepth", "1", "-type", "d"])).stdout.trim().split("\n");
  ...
  await run("sudo", ["systemctl", "stop", "hynix-spread.service"]).catch(() => {});
  assert.ok(resolve(scratch).startsWith(resolve(tmpdir()) + "/hynix-installer-test-"));
