...
export default function AlertSettings() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      pending = true;
      try {
        const response = await fetch("/api/alerts", { cache: "no-store", signal: AbortSignal.any([controller.signal, AbortSignal.timeout(15_000)]) });
  ...
  async function save(event: FormEvent) {
    ...
    try {
      const response = await fetch("/api/alerts", { method: "PUT", headers: { "Content-Type": "application/json" }, signal: AbortSignal.timeout(25_000), body: JSON.stringify({ ...draft, revision, webhookUrl, signingSecret, clearSigningSecret }) });
  ...
  async function testMessage() {
    ...
    try {
      const response = await fetch("/api/alerts/test", { method: "POST", headers: { "Content-Type": "application/json" }, body: "{}", signal: AbortSignal.timeout(25_000) });
      const result = await response.json() as AlertView & { error?: string };
