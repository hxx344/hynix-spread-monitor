...
rollback() {
  log '启动未通过检查，正在恢复原服务。'
  systemctl stop hynix-spread.service || return 1
  ...
  if (( was_active )); then systemctl start hynix-spread.service || return 1; fi
}
...
finish() {
  ...
  if (( status != 0 && switching )); then
    rollback || { rollback_failed=1; printf '自动恢复未完成，请查看 systemctl status hynix-spread。\n' >&2; }
  fi
  if (( status != 0 )); then
    printf '安装未完成；配置和告警数据已保留。日志：journalctl -u hynix-spread -n 50\n' >&2
    if (( ! rollback_failed )) && [[ -n "$release" && "$release" == "$base/releases/"* && $(readlink -f "$base/current") != "$release" ]]; then
      rm -rf -- "$release"
    fi
  fi
  if [[ -n "$scratch" && "$scratch" == /tmp/hynix-install.* ]]; then rm -rf -- "$scratch"; fi
  exit "$status"
}
...
main() {
  set -Eeuo pipefail
  umask 022
  base=/opt/hynix-spread
  config=/etc/hynix-spread.env
  unit=/etc/systemd/system/hynix-spread.service
  ...
  mkdir -p /run/lock
  exec 9>/run/lock/hynix-spread-install.lock
  flock -n 9 || die '已有部署正在运行，请等待结束'
  scratch=$(mktemp -d /tmp/hynix-install.XXXXXXXX)
  trap finish EXIT
  trap 'exit 130' INT
  trap 'exit 143' TERM
  ...
  if ! id hynix >/dev/null 2>&1; then
    useradd --system --user-group --home-dir "$base" --shell /usr/sbin/nologin hynix
  fi
  getent group hynix >/dev/null || die '已有 hynix 用户缺少同名用户组'
  install -d -m 0700 -o hynix -g hynix /var/cache/hynix-spread
  if [[ ! -e "$config" ]]; then
    first_install=1
    install -m 0600 /dev/null "$config"
    {
      ...
      printf 'ALERT_DATA_DIR=/var/lib/hynix-spread\n'
    } > "$config"
    install -m 0600 /dev/null "$base/.credentials-unshown"
  elif [[ -n "$requested_port" ]]; then
    log '检测到已有配置，保留原端口；如需更改，请编辑 /etc/hynix-spread.env。'
  fi

  log '正在下载源码并构建，新版本准备完成后才会切换服务。'
  if [[ -z "$source_dir" ]]; then
    download 'https://api.github.com/repos/hxx344/hynix-spread-monitor/commits/main' "$scratch/commit.json"
    ...
    download "https://codeload.github.com/hxx344/hynix-spread-monitor/tar.gz/$release_id" "$scratch/source.tar.gz"
  else
    release_id=local
  ...
  [[ -f "$release/deploy/check-install.mjs" && -f "$release/deploy/hynix-spread.service" ]] || die '发布文件不完整'
  systemd-run --quiet --wait --pipe --collect --unit="hynix-spread-config-$$" --property="EnvironmentFile=$config" "$runtime/bin/node" "$release/deploy/check-install.mjs" --config-only
  ln -s "$runtime" "$release/.runtime"
  chown -R hynix:hynix "$release"
  (
    cd "$release"
    runuser -u hynix -- env PATH="$runtime/bin:$PATH" NODE_ENV=development NEXT_TELEMETRY_DISABLED=1 npm_config_cache=/var/cache/hynix-spread npm ci --include=dev --include=optional --no-audit --no-fund
    runuser -u hynix -- env PATH="$runtime/bin:$PATH" NODE_ENV=production NEXT_TELEMETRY_DISABLED=1 npm run build:linux
  )
  [[ -f "$release/.next/BUILD_ID" ]] || die '构建未生成可启动版本'
  sed -e "s|^WorkingDirectory=.*|WorkingDirectory=$base/current|" -e "s|^ExecStart=.*|ExecStart=$base/current/.runtime/bin/node --experimental-strip-types $base/current/server/linux.mjs|" "$release/deploy/hynix-spread.service" > "$scratch/hynix-spread.service"
  systemd-analyze verify "$scratch/hynix-spread.service" 2> "$scratch/unit-check.log" || {
    # The current symlink is intentionally not switched yet. Verify using the prepared release.
    sed "s|$base/current|$release|g" "$scratch/hynix-spread.service" > "$scratch/check.service"
  ...
  if systemctl is-active --quiet hynix-spread.service; then was_active=1; fi
  switching=1
  systemctl stop hynix-spread.service 2>/dev/null || { (( ! was_active )) || die '无法停止原服务'; }
  ln -s "$release" "$base/current.next"
  mv -Tf "$base/current.next" "$base/current"
  install -m 0644 "$scratch/hynix-spread.service" "$unit"
  systemctl daemon-reload
  systemctl start hynix-spread.service
  # Read EnvironmentFile through systemd itself; never execute or echo an existing config.
  systemd-run --quiet --wait --pipe --collect --unit="hynix-spread-check-$$" --property="EnvironmentFile=$config" "$runtime/bin/node" "$release/deploy/check-install.mjs"
  systemctl is-active --quiet hynix-spread.service || die '服务未保持运行'
  local main_pid
  main_pid=$(systemctl show --property=MainPID --value hynix-spread.service)
  [[ "$main_pid" =~ ^[1-9][0-9]*$ && $(readlink -f "/proc/$main_pid/cwd") == "$release" ]] || die '运行进程与新版本不一致'
  systemctl enable hynix-spread.service
  switching=0

  log '部署完成，已启用开机启动和每 10 秒后台告警检查。'
  [[ ! -f "$base/.credentials-unshown" ]] || first_install=1
  systemd-run --quiet --wait --pipe --collect --unit="hynix-spread-info-$$" --property="EnvironmentFile=$config" "$runtime/bin/node" "$release/deploy/check-install.mjs" --describe "$first_install"
  rm -f -- "$base/.credentials-unshown"
  printf '配置文件：%s\n服务日志：sudo journalctl -u hynix-spread -f\n升级：再次运行相同的安装命令。\n' "$config"
}
